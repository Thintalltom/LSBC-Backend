...
def update_player(player_id, data):
    ...
    result = db.players.update_one(
        {"_id": ObjectId(player_id)}, {"$set": data})
...
def get_individual_club(club_id):
    club = db.clubs.find_one({"_id": ObjectId(club_id)})
    if club:
        club["_id"] = str(club["_id"])
        players = list(db.players.find({"club_id": ObjectId(club_id)}))
        for player in players:
            player["_id"] = str(player["_id"])
            player["club_id"] = str(player["club_id"])
        coaches = list(db.coaches.find({"club_id": ObjectId(club_id)}))
        for coach in coaches:
            coach["_id"] = str(coach["_id"])
            coach["club_id"] = str(coach["club_id"])
        club["players"] = players
